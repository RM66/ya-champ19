module.exports = function(ast) {
  ...
  traverse(ast, (node) => {
    if (node.type === 'ObjectProperty' &&
      node.key && node.key.name === 'a' && node.value && node.value.value === '') {
      aIsEmptyStr = true;
    } else if (node.type === 'Identifier' && node.name === 'w' && aIsEmptyStr) {
      result.push(node);
    }
  }, (node) => {
    if (node.type === 'ExpressionStatement') aIsEmptyStr = null;
  });
