  ...
  traverse(
    ...
    node => {
      if (
        ...
        aIsString = true;
      } else if (node.type === "MemberExpression" && aIsString) {
        let prop = node.property;
        if (
          (prop.type === "Identifier" && prop.name === "w") ||
          (prop.type === "StringLiteral" && prop.value === "w")
        ) {
          result.push(prop);
          aIsString = null;
        }
    ...
    if (Array.isArray(astNode)) {
      ...
      const innerScope = resolveScope(astNode, scope),
        keys = Object.keys(astNode).filter(key => {
          ...
          return (
            key !== "loc" && astNode[key] && typeof astNode[key] === "object"
          );
        });
...
Scope.prototype = {
  ...
  isDefined(name) {
    return (
      this._vars.has(name) || (this._parent && this._parent.isDefined(name))
    );
  }
};
